...
**Axis A — native**: Does the tool natively discover and invoke files placed in the listed directory? Values: `Yes`, `No`, `Partial`, `Unknown`. `Partial` means some native support exists but with limitations. `Unknown` means native behaviour has not been confirmed through testing or available documentation.
...
| Claude Code | instructions | Partial | `native:.claude/rules/` | install-first | [verified] | APM deploys to `.claude/rules/` on install; `apm compile` additionally emits `CLAUDE.md` (the canonical file Claude Code reads); `.claude/rules/` is a newer path introduced by APM |
...
| Cursor | hooks | Unknown | `native:.cursor/hooks.json` | unsupported | [verified] | APM deploys to `.cursor/hooks.json` (observed in test run); Cursor's native consumption of hooks.json is not confirmed in public docs — Axis A unknown, so cannot classify as install-first |
...
| Codex CLI | hooks | Unknown | `native:.codex/hooks.json` | unsupported | [verified] | APM deploys to `.codex/hooks.json` (observed in test run); Codex's native consumption of hooks.json is not confirmed in public docs — Axis A unknown, so cannot classify as install-first |
...
| Gemini CLI | instructions | Partial | `untested` | unsupported | [verified] | Gemini CLI reads `GEMINI.md` as its instruction file (analogous to CLAUDE.md) [doc]; `apm compile --target all` was run and did not produce GEMINI.md in 0.8.12 [verified] |
